...
static void on_entry(HSM_state_t * state) {
    data_t * data = (data_t *) state->data;
    data->entered = true;
}

static void on_exit(HSM_state_t * state) {
    data_t * data = (data_t *) state->data;
    data->exited = true;
}

static void on_initialize(HSM_state_t * state) {
    data_t * data = (data_t *) state->data;
    data->initialized = true;
}

static void reset_state(HSM_state_t * state) {
    data_t * data = (data_t *) state->data;
    data->entered = data->exited = data->initialized = false;
}

static HSM_state_t build_state(char const * name, HSM_state_t * parent, void * data) {
    HSM_state_t s = {
        .name = name,
        .parent = parent,
        .on_entry = on_entry,
 ...
 */
void test_transition_to_reachable_with_initialize(void) {
    // Set up
...
void test_transition_to_unreachable_with_initialize(void) {
    ...
    TEST_ASSERT_FALSE(dest_data.exited);
    TEST_ASSERT_FALSE(dest_data.exited);
    TEST_ASSERT_FALSE(dest_data.initialized);
    TEST_ASSERT_FALSE(unreachable_data.entered);
    TEST_ASSERT_FALSE(unreachable_data.exited);
    TEST_ASSERT_FALSE(unreachable_data.initialized);
}